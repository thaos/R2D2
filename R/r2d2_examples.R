...
#' \describe{
#'   \item{bc1d_cal}{A matrix with 589 rows and 8334 columns. It contains the data during the calibration period to be corrected by the r2d2 method.}
#'   \item{bc1d_val}{A matrix with 589 rows and 8334 columnss. It contains the data during the validation period to be corrected by the r2d2 method.}
#'   \item{ref_cal}{A matrix with 589 rows and 8334 columns. It contains the reference data during the calibration period. The r2D2 reproduces rank associations observed in this dataset depending on the conditioning dimension values present in in the bc1d dataset to be corrected.}
#'   \item{ref_val}{A matrix with 589 rows and 8334 comumns. It contains the reference data during the validation period. It is used to test and see how r2d2 performs as a bias correction method.}
#'   For those four matrices, the number of rows corresponds to the number of observations or time-steps. The number of columns corresponds to the numbers of variables to be corrected. The first 4167 columns corresponds to temperature variables at different geographical sites. The following 4167 columns corresponds to precipitaion variables for the same site as for the temperature.
...
#'   \item{dates_cal}{vector of strings with dates used for the calibration datasets. For this examples, the calibration datasets only consist of days in January during the 1979-1997 period.}
#'   \item{dates_val}{vector of strings with dates used for the validation datasets. For this examples, the calibration datasets only consist of days in January during the 1998-2016 period.}
